-- Fix admin_delete_game_cascade: session_games uses session_id, not game_id
...
BEGIN
  ...
  -- Delete Lion game data (use session_id for session_games lookup)
  ...
  DELETE FROM lion_game_state WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);

  -- Delete adventure data
  DELETE FROM adventure_scores WHERE session_id = p_game_id;
  DELETE FROM adventure_game_configs WHERE game_id = p_game_id;
  DELETE FROM stage_scores WHERE session_id = p_game_id;
  ...
  -- Delete Sheriff game data
  DELETE FROM sheriff_duels WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);
  DELETE FROM sheriff_player_choices WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);
  DELETE FROM sheriff_round_state WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);

  -- Delete River game data
  DELETE FROM river_level_history WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);
  DELETE FROM river_session_state WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);

  -- Delete session events
  DELETE FROM session_events WHERE session_game_id IN (SELECT id FROM session_games WHERE session_id = p_game_id);

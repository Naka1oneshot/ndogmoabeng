...
interface ComingSoonRegistrationModalProps {
  open: boolean;
...
export function ComingSoonRegistrationModal({ 
  open, 
  onOpenChange, 
  event,
  onSuccess 
}: ComingSoonRegistrationModalProps) {
  ...
  const sendCallbackNotification = async () => {
    try {
      await supabase.functions.invoke('notify-callback-request', {
        body: {
          ...
          phone: phone.trim(),
          companionsCount,
          companionsNames: companionsNames.filter(n => n.trim()),
      ...
      console.error('Failed to send callback notification:', err);
    }
  };
      ...
      <DialogContent className="sm:max-w-md border-border max-h-[90vh] overflow-y-auto" style={{ backgroundColor: 'hsl(var(--card))' }}>
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Remplis ce formulaire pour t'inscrire à « {event.title} »
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          {/* ACTION BUTTONS - Callback button is PRIMARY */}
          <div className="space-y-3">
            {/* PRIMARY: Callback / Cash payment button */}
            <Button
              type="button"
            ...
            {!isFreeEvent && (
              <>
                ...
                <Button
                  ...
                  className="w-full h-11 border-primary/50 hover:bg-primary/10 hover:border-primary text-foreground"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />

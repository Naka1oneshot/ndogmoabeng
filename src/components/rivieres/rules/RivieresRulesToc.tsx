...
interface RivieresRulesTocProps {
  open: boolean;
  onClose: () => void;
...
export function RivieresRulesToc({
  open,
  onClose,
  pages,
  currentIndex,
  onSelect,
}: RivieresRulesTocProps) {
  if (!open) return null;
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-[#0B1020] border border-[#D4AF37]/30 rounded-xl w-full max-w-md mx-4 overflow-hidden"
                ...
                <motion.button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
                    i === currentIndex
                      ...
                      : 'bg-[#1a1f2e] border border-transparent hover:border-[#D4AF37]/30'
                  }`}
                >
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold ${
                    i === currentIndex
                      ? 'bg-[#D4AF37] text-[#0B1020]'

...
import { DynamicSection, DynamicText } from '@/components/rules/DynamicSection';

interface DynamicRulesQuickPage1Props {
...
export function DynamicRulesQuickPage1({ context, replayNonce }: DynamicRulesQuickPage1Props) {
  const { getSection, getParagraphs, loading } = useDynamicRules('RIVIERES');
  const section = getSection('quick_objectif');

  // Fallback to static content while loading
  if (loading || !section) {
    return (
      <motion.div
        key={replayNonce}

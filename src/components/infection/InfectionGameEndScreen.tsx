...
import { Trophy, Skull, Syringe, FlaskConical, Shield } from 'lucide-react';
...
export function InfectionGameEndScreen({ gameId, sessionGameId, player }: InfectionGameEndScreenProps) {
  ...
  const fetchData = async () => {
    ...
    if (victoryEvent) {
      // Extract winner from message or payload
      const payload = victoryEvent.payload as { winner?: string } | null;
  ...
  // Sort players: winners first, then by role
  const sortedPlayers = [...allPlayers].sort((a, b) => {
    const aWon = (winner === 'SY' && a.team_code === 'SY') || (winner === 'PV' && a.team_code === 'PV');
    const bWon = (winner === 'SY' && b.team_code === 'SY') || (winner === 'PV' && b.team_code === 'PV');
      ...
      <div className={theme.card}>
        ...
        <div className="divide-y divide-[#2D3748]">
          {sortedPlayers.map(p => {
            ...
            const pWon = (winner === 'SY' && p.team_code === 'SY') || (winner === 'PV' && p.team_code === 'PV');
